...
def lone_object_filter(image, min_size=2, connectivity=1, kernel_size=3):
    ...
    Args:
        image (numpy.array): The image to filter.
        min_size (int): Defines the minimum number of contiguous pixels that
                        will not be filtered out.
        connectivity (int): The maximum distance between any two pixels such
                            that they are considered one group.
                            For example, a connectivity of 1 considers only
                            adjacent value to be within one group, but a
                            connectivity of 2 also considers diagonally
                            connected values to be within one group.
        kernel_size (numeric):
                            The diameter of the circular kernel to use for
                            filling NaN values with a modal filter.
                            If more isolated, contiguous regions
